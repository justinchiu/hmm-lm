...
#for eps in [1e-3]:
    ...
    # true dist is one-hot + smoothing
    def true_dist_onehot(num_classes):
        logits = torch.eye(num_classes, device=device)
        logits.diagonal(offset=1).fill_(1)
        logits = (logits + eps).log()
        true_dist = logits.log_softmax(-1)
        print(f"True dist H: {H(true_dist).mean().item():.2f}")
        return true_dist
    run_fit(true_dist_onehot)
    ...
    print("Smoothed 3-hot True Dist")
    # true dist is one-hot + smoothing
    def true_dist_onehot(num_classes):
        logits = torch.eye(num_classes, device=device)
        logits.diagonal(offset=1).fill_(1)
        logits.diagonal(offset=2).fill_(1)
        logits = (logits + eps).log()
        true_dist = logits.log_softmax(-1)
        print(f"True dist H: {H(true_dist).mean().item():.2f}")
        return true_dist
