...
def get_logmvv_fwd(NT, C, DD):
    ...
    with autotvm.apply_history_best('hmm_runners/mvv.log'):
...
@autotvm.template("logsummulouterback")
def logsummulouterback(n, l, m, dtype):
    #nn = n
    #n = nn
    #bb = 1
    #b = bb
    ...
    def grad_output1(bb, m1, ll):
        ...
        return te.sum(grad_B[bb, m1, m2, ll], axis=m2)
    def grad_output2(bb, m2, ll):
        #return te.sum(grad_B[bb, m1, m2, ll] * B1[bb, m1, ll], axis=m1)
        return te.sum(grad_B[bb, m1, m2, ll], axis=m1)
    output = grad_B
...
S1, S2, S3 = 16, 16, 16
def get_logmvv_bwd(NT, C, DD):
    S1, S2, S3 = NT, C, DD
    # print(tvm.lower(*logsummulback(S1, S2, S3, 'float32'), simple_mode=True))
    from tvm.contrib.dlpack import to_pytorch_func
    task = autotvm.task.create("logsummulouterback", args=(S1, S2, S3, 'float32',), target='cuda', target_host="llvm")
    with autotvm.apply_history_best('hmm_runners/mvv.log'):
        #with tvm.target.Target("cuda"):
        with tvm.target.create("cuda"):
            s_mult, arg_bufs = logsummulouterback(S1, S2, S3, 'float32')
            mod = tvm.build(s_mult, arg_bufs)
    logsumback_pytorch = to_pytorch_func(mod)
    return logsumback_pytorch
...
if __name__ == "__main__":
    ...
    S2 = 1024
    S3 = 512*512
    logsumouter_pytorch = get_logmvv_fwd(S1, S2, S3)
    logsumouterback_pytorch = get_logmvv_bwd(S1, S2, S3)
    X = torch.rand(S1, 1, S2).cuda()
    Y1 = torch.rand(S1, int(math.sqrt(S3)), S2).cuda()
    Y2 = torch.rand(S1, int(math.sqrt(S3)), S2).cuda()
    Y1.requires_grad_(True)
    Y2.requires_grad_(True)
    out = torch.rand(S1, S3, 1).cuda()
    M = torch.rand(S1, S3, 1).cuda()

    logsumouter_pytorch(X, Y1, Y2, out, M)
    import pdb; pdb.set_trace()
    b = ((Y1[:, :, None, :] + Y2[:, None, :, :]).view(S1, 1, S3, S2) + X.view(S1, 1, 1, S2)).logsumexp(-1)

    grad_C = torch.rand(S1, S3, 1).cuda()
    grad_B1 = torch.rand(S1, int(math.sqrt(S3)), S2).cuda()
    grad_B2 = torch.rand(S1, int(math.sqrt(S3)), S2).cuda()
    logsumouterback_pytorch(X, Y1, Y2, out, M, grad_C, grad_B1, grad_B2)

    import pdb; pdb.set_trace()

    print(torch.allclose(out, b.transpose(1, 2)))
