...
def get_logmm_fwd(NT, C, DD):
    task = autotvm.task.create("logsummul",
        #args=(512, 256, 512*512, 'float32',),
        args=(NT, C, DD, 'float32',),
        #args=(DD, NT, C, 'float32',),
        target='cuda', target_host="llvm")
    with autotvm.apply_history_best('hmm_runners/matmul.log'):
        #with tvm.target.Target("cuda"):
        with tvm.target.create("cuda"):
            s_mult, arg_bufs = logsummul(DD, C, NT, 'float32')
            mod = tvm.build(s_mult, arg_bufs)

    logsum_pytorch = to_pytorch_func(mod)
    return logsum_pytorch

def get_logmm_bwd(NT, C, DD):
    task = autotvm.task.create("logsummulback",
        #args=(512, 256, 512*512, 'float32',),
        args=(NT, C, DD, 'float32',),
        #args=(DD, NT, C, 'float32',),
        target='cuda', target_host="llvm")
    with autotvm.apply_history_best('hmm_runners/matmul.log'):
        #with tvm.target.Target("cuda"):
        with tvm.target.create("cuda"):
            s_mult, arg_bufs = logsummulback(DD, C, NT, 'float32')
            mod = tvm.build(s_mult, arg_bufs)
...
if __name__ == "__main__":
    ...
    for S1,S2,S3 in sizes:
        ...
        task = autotvm.task.create("logsummul", args=(S3, S2, S1,'float32',), target='cuda', target_host="llvm")
        print(task.config_space)
        ...
        task = autotvm.task.create("logsummulback", args=(S3, S2, S1,'float32',), target='cuda', target_host="llvm")
        print(task.config_space)
        measure_option = autotvm.measure_option(
            builder=autotvm.LocalBuilder(n_parallel=5),
            runner=autotvm.LocalRunner(number=5, repeat=2, timeout=60, min_repeat_ms=50))
