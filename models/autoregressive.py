import torch
from torch_struct.semirings import MaxSemiring, KMaxSemiring, TempMax
...
class Autoregressive(Distribution):
    ...
    * event_shape -> N x T sequence of choices
    ...
    def log_prob(self, value, sparse=False, reduce=False):
        """
        ...
        scores = log_probs[:, :-1].gather(2, value[:, 1:].unsqueeze(-1))
        if reduce:
