...
# Multiple projection Sparse Bmm Linear Hmm Language model
class MsblHmmLm(nn.Module):
    def __init__(self, V, config):
        super(MsblHmmLm, self).__init__()
    ...
    def init_partitions(self, config):
        ...
        self.register_buffer("state2cluster",
            th.arange(self.num_clusters).repeat_interleave(self.states_per_word))
        self.register_buffer("state2cluster_d",
            th.arange(self.num_clusters).repeat_interleave(self.states_per_word_d))
    ...
    def score(self, text, lpz=None, last_states=None, mask=None, lengths=None):
        ...
        word2cluster = self.word2cluster
    ...
    def score_rff(self, text, lpz=None, last_states=None, mask=None, lengths=None):
        ...
        if self.training and self.dropout_type != "none":
            ...
            word2state = self.word2state_d
            state2cluster = self.state2cluster_d
            states_per_word = self.states_per_word_d
        else:
            states = None
            feat_mask = None
            word2state = self.word2state
            state2cluster = self.state2cluster
            states_per_word = self.states_per_word
        word2cluster = self.word2cluster
        ...
        for t in range(T-1):
            ...
            gamma = th.einsum("ns,nsd->nd", alpha, left_proj[:,t])
            alpha_un = logp_emit[:,t+1] + th.einsum(
    ...
    def compute_rff_parameters(self):
        ...
        # O(CD)
        log_denominator = (
            log_phi_w.view(self.num_clusters, self.states_per_word, self.D)
            + log_phi_u.logsumexp(1, keepdim=True)
        ).logsumexp(-1).view(self.C)
        # O(CD)
        normed_log_phi_w = log_phi_w - log_denominator.unsqueeze(-1)
    ...
    def compute_rff_loss(
        ...
        # N x T x C
        state_t = word2state[text]
        cluster_t = self.word2cluster[text]
        logp_emit = emission[
            state_t,
            text[:,:,None],
        ]
